# Google Cloud Function Source Code for Send Enquiry

## Function URL
```
https://send-enquiry-541421913321.europe-west4.run.app
```

## Source Code to Copy to Google Cloud Console

### File: `main.py`

Copy this entire code into the Google Cloud Console function editor:
...
def send_enquiry(request):
    ...
    Expected JSON payload:
    {
        ...
        "enquiryType": "Seller Registration"   // optional; when set, email subject becomes "Enquiry : {enquiryType}"
    }
    """
    ...
    enquiryType = request_json.get('enquiryType', '').strip()
    
    logger.info(f"Send enquiry request from {email}")

    # Create email
    message_obj = MIMEMultipart()
    message_obj['From'] = gmail_address
    message_obj['To'] = 'enquiries@internationalpropertymarket.com'
    message_obj['Subject'] = f'Enquiry : {enquiryType}' if enquiryType else f'New Inquiry from {firstName} {lastName}'
    ...
    try:
        ...
        logger.info(f"Enquiry email sent successfully to enquiries@internationalpropertymarket.com from {email}")
...
Copy this into the requirements.txt file in Google Cloud Console:
...
1. **Function Name**: `send-enquiry`
2. **Entry Point**: `send_enquiry`
...
Make sure these are set in the Google Cloud Function:
- `GMAIL_ADDRESS`: Your Gmail address (same as OTP function)
- `GMAIL_APP_PASSWORD`: Your Gmail App Password (same as OTP function)

## Testing

After updating the function, test it with:

```bash
curl -X POST https://send-enquiry-541421913321.europe-west4.run.app \
