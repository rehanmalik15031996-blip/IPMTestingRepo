...
export function PreferencesProvider({ children }) {
  ...
  const setLanguage = useCallback((lang) => {
    // Write preference synchronously to localStorage BEFORE the reload so the
    // early script in index.html can read it on the next page load and set the
    // googtrans cookie + notranslate class before React even mounts.
    try {
      const existing = JSON.parse(localStorage.getItem(STORAGE_KEY) || '{}');
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ ...existing, language: lang, languageExplicitlySet: true }));
    } catch (e) { /* ignore */ }
    // Also update React state (handles the rare case where reload is blocked)
    setPrefsState((p) => ({ ...p, language: lang, languageExplicitlySet: true }));
    i18n.changeLanguage(lang);
    // Reload the page so Google Translate is applied cleanly from page load via
    // the googtrans cookie — far more reliable than trying to translate in-place,
    // especially on Vercel where in-page widget manipulation can be flaky.
