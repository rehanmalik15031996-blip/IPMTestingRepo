...
import VaultLegacy from './legacy/VaultLegacy';
...
const Vault = () => {
    ...
    const [files, setFiles] = useState(() => {
        try {
            ...
            const c = u?._id ? getVaultCache(u._id) : null;
            return c?.files ?? [];
        } catch (_) {
            return [];
        }
    ...
    const [limitBytes] = useState(1024 * 1024 * 1024); // 1GB
    ...
    const [uploadDocumentType, setUploadDocumentType] = useState('general'); // 'general' | 'property'
    ...
    // Buyers, sellers, investors, and tenants use the legacy version of Vault (after all hooks)
    ...
    const handleFileChange = async (e) => {
        ...
        // MongoDB Atlas: allow large documents and images (50MB per file)
    ...
    const comingSoonMessage = "You will soon have access to the IPM Smart Vault, where IPM AI will help you manage all the documents you need for managing your properties";
    const buyerBlurStyle = isBuyer
        ? { filter: 'blur(2px)', pointerEvents: 'none', userSelect: 'none', flex: 1, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' }
        : { flex: 1, minHeight: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column' };
    const BuyerBlur = ({ children }) => <div style={buyerBlurStyle}>{children}</div>;
    ...
    return (
        <div className="dashboard-container" style={{ display: 'flex' }}>
            ...
            {/* MAIN CONTENT - fixed height so content scrolls inside, does not extend page */}
            <main className="dashboard-main" style={{ flex: 1, padding: isMobile ? '16px' : '30px', backgroundColor: '#f4f7f6', height: '100vh', maxHeight: '100vh', overflow: 'hidden', position: 'relative', display: 'flex', flexDirection: 'column' }}>
                {isBuyer && (
                    <div style={{
                        background: '#f8fafc',
                ...
                <div style={{ marginBottom: '16px', background: 'white', borderRadius: '12px', padding: '12px 16px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', fontSize: '13px', color: '#555' }}>
                        ...
                        <span style={{ fontWeight: '600', color: '#11575C' }}>
                            {(usedBytes / (1024 * 1024)).toFixed(2)} MB / {(limitBytes / (1024 * 1024)).toFixed(0)} MB
                            {' · '}{limitBytes ? Math.round((usedBytes / limitBytes) * 100) : 0}% used
                        </span>
                    </div>
                ...
                <header className="dash-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                    ...
                    <button
                        ...
                        onClick={() => {
                            ...
                            if (userId) {
                                api.get(`/api/users/${userId}?type=dashboard`).then((r) => {
                ...
                <BuyerBlur>
                <div className="vault-grid" style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? '16px' : '30px', flex: 1, minHeight: 0, overflow: 'hidden' }}>
                    
                    {/* LEFT COLUMN (Folders) - scrollable list; hidden on mobile */}
                    <div className="vault-left-col" style={{ width: isMobile ? '100%' : '280px', minWidth: isMobile ? 0 : '280px', display: isMobile ? 'none' : 'flex', flexDirection: 'column', background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 15px rgba(0,0,0,0.05)', overflow: 'hidden' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <select
                                ...
                                onChange={(e) => { setUploadDocumentType(e.target.value); if (e.target.value === 'general') { setUploadPropertyId(''); setUploadPropertyTitle(''); } }}
                                style={{ width: '100%', padding: '10px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', background: 'white' }}
                            >
                        ...
                        {uploadDocumentType === 'property' && (
                            <div style={{ marginBottom: '20px' }}>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        const o = propertyOptions.find(p => String(p.id) === e.target.value);
                                        setUploadPropertyId(e.target.value);
                                        setUploadPropertyTitle(o ? o.title : '');
                                    }}
                                    style={{ width: '100%', padding: '10px', border: '1px solid #e5e7eb', borderRadius: '8px', fontSize: '14px', background: 'white' }}
                                >
                        ...
                        <div style={{ marginBottom: '20px' }}>
                            ...
                            <div
                                ...
                                onDrop={(e) => {
                                    ...
                                    if (usedBytes + file.size > limitBytes) {
                                        showNotification(`Storage limit (1 GB) would be exceeded.`, 'error');
                                        return;
                                    }
                                ...
                                style={{
                                    border: `2px dashed ${dragOver ? '#11575C' : '#e5e7eb'}`,
                                    borderRadius: '12px',
                                    padding: '28px',
                                    textAlign: 'center',
                                    background: dragOver ? 'rgba(17, 87, 92, 0.06)' : '#f9fafb',
                                    cursor: 'pointer',
                                    transition: '0.2s'
                                }}
                            >
                                <i className="fas fa-cloud-upload-alt" style={{ fontSize: '32px', color: '#11575C', marginBottom: '8px' }}></i>
                                <div style={{ fontSize: '14px', color: '#555' }}>Drag and drop a file here, or click to choose</div>
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                            <button
                                onClick={() => {
                                    setShowUploadModal(false);
                                    setUploadFolder('General Documents');
                                    setUploadDocumentType('general');
                                    setUploadPropertyId('');
                                    setUploadPropertyTitle('');
                                    if (fileInputRef.current) fileInputRef.current.value = '';
                                }}
                                style={{ padding: '10px 20px', border: '1px solid #e5e7eb', borderRadius: '8px', background: 'white', cursor: 'pointer', fontSize: '14px', fontWeight: '600', color: '#333' }}
                            >
                                Cancel
                            </button>
                            <button
                                onClick={() => fileInputRef.current?.click()}
                                disabled={uploading}
                                style={{ padding: '10px 20px', border: 'none', borderRadius: '8px', background: uploading ? '#A7ABAC' : '#11575C', cursor: uploading ? 'not-allowed' : 'pointer', fontSize: '14px', fontWeight: '600', color: 'white' }}
                            >
                                {uploading ? 'Uploading...' : 'Add from local'}
                            </button>
                        </div>
                    </div>
                </div>
