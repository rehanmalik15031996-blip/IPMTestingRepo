...
def send_enquiry(request):
    ...
    # Browsers / uptime checks often GET the URL — enquiries use POST only.
    if request.method == 'GET':
        return (
            jsonify({
                'success': True,
                'service': 'send-enquiry',
                'hint': 'Website enquiries: POST JSON with firstName, lastName, email, phone, message',
            }),
            200,
            headers,
        )
