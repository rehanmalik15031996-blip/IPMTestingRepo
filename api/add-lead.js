// Vercel serverless: POST /api/add-lead – add a CRM lead for agents (top-level to avoid conflict with api/users/[id])
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const agencyDoc = isAgency ? null : (role === 'agency_agent' && user.agencyId ? await User.findById(user.agencyId).lean() : null);
    ...
    const newLead = {
      ...
      leadType: (lead.leadType === 'seller' || lead.leadType === 'investor') ? lead.leadType : 'buyer',
      ...
      investorDetails: lead.leadType === 'investor' && lead.investorDetails ? lead.investorDetails : undefined
    };
    ...
    if (isAgency) {
      ...
      crmLeads = user.agencyStats.crmLeads;
    } else if (role === 'agency_agent' && user.agencyId) {
      ...
      crmLeads = agency.agencyStats.crmLeads.filter((l) => String(l.assignedAgentId || '') === String(user._id));
    } else {
